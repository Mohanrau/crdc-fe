<template>
    <div class="general-page-holder">
        <general-header generalTitle="List of Campaigns"></general-header>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        campaignSearchData: state => state.globalCMP.campaignSearchDataGlobal
      }),
      ...mapGetters({
        tableList: 'tableDataCampaigns',
    ...
    data: () => ({
      ...
      columns: [
        {
          name: 'country',
          label: Vue.prototype.$t('campaigns.campaign.country'),
          field: 'country',
          sortable: true,
          format (value) {
            if (value !== '') {
        ...
        {
          name: 'name',
          label: Vue.prototype.$t('campaigns.campaign.name'),
          field: 'name',
          sortable: true,
          required: true
        },
    ...
    methods: {
      ...
      queryHandler (data) {
        ...
        this.getCampaignListAction(_(this.query).omitBy(_.isUndefined).omitBy(_.isNull).value()).then(() => {
          // Remove observers
      ...
      actions (row, type) {
        if (type === 'create') {
          ...
          this.$router.push({name: 'Update Campaign', params:{campaignId: row.id}})
        } else if (type === 'delete') {
          Dialog.create({
            title: 'Warning',
            ...
            this.deleteCampaignAction(row.id).then(response => {
              this.filterChange()
            }).catch(error => {
              if (typeof error.response !== 'undefined' && error.response.status === 422) {
                if (typeof error.response.data !== 'undefined' && typeof error.response.data.id !== 'undefined') {
                  this.setNotification({
                    title: 'eSac Campaign Deletion Failed',
      ...
      routePage (data) {
        this.$router.push({name: 'Update Campaign', params:{campaignId: data.id}})
      },
      openForm () {
        this.$router.push({name: 'New Campaign'})
      }
