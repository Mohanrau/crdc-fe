<template>
    <div class="general-page-holder">
        <general-header generalTitle="List of eSac Voucher Type"></general-header>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        ...
        esacVoucherTypeSearchData: state => state.globalCMP.esacVoucherTypeSearchDataGlobal
      }),
      ...mapGetters({
        tableList: 'tableDataEsacVoucherTypes',
      ...
      columns: [
        {
          name: 'created_at',
          align: 'center',
          label: Vue.prototype.$t('Common.DTableLabel.CreationDate'),
          field: 'created_at',
          sortable: true,
          format (value) {
            if (value !== null) {
        ...
        },
        {
          name: 'country',
          align: 'center',
          label: Vue.prototype.$t('campaigns.esacVoucherType.country'),
          field: 'country',
          sortable: true,
          format (value) {
            if (value !== '') {
        ...
        },
        {
          name: 'name',
          align: 'center',
          label: Vue.prototype.$t('campaigns.esacVoucherType.name'),
          field: 'name',
          sortable: true,
          required: true
        },
        {
          name: 'description',
          align: 'left',
          label: Vue.prototype.$t('campaigns.esacVoucherType.description'),
          field: 'description',
          sortable: true,
          required: true
        },
    ...
    methods: {
      ...
      filterChange () {
        this.queryHandler(this.setDefaultPaginationAttr('esac-voucher-type'))
      },
      actions (row, type) {
        if (type === 'create') {
          this.$router.push({ name: 'New eSac Voucher Type' })
        } else if (type === 'update') {
          this.$router.push({ name: 'Update eSac Voucher Type', params: { esacVoucherTypeId: row.id }})
        } else if (type === 'delete') {
          Dialog.create({
            title: 'Warning',
            ...
            this.deleteEsacVoucherTypeAction(row.id).then(response => {
              this.filterChange()
            }).catch(error => {
              if (typeof error.response !== 'undefined' && error.response.status === 422) {
                if (typeof error.response.data !== 'undefined' && typeof error.response.data.id !== 'undefined') {
                  this.setNotification({
                    title: 'eSac Voucher Type Deletion Failed',
      ...
      routePage (data) {
        this.$router.push({ name: 'Update eSac Voucher Type', params: { esacVoucherTypeId: data.id }})
      },
      openForm () {
        this.$router.push({ name: 'New eSac Voucher Type' })
      }
