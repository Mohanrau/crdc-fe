<template>
  <div class="sales-shipping-tab">
            ...
            <div class="row items-center gutter-sm">
              <div class="col-lg-auto">
                <q-field :error-label="$t('Common.address.errorLabel.selectShippingType')"
                         :error="validations.exchangeData.selected.shipping.sale_delivery_method.$error">
                  <q-option-group
                    :disable="lockProductExchange"
                    color="secondary"
                    type="radio"
                    v-model="exchangeData.selected.shipping.sale_delivery_method"
                    inline
                    @input="initAddressMixin(), validations.exchangeData.selected.shipping.sale_delivery_method.$touch"
    ...
    <div class="sales-shipping-content"
         v-show="exchangeData.selected.shipping.sale_delivery_method !== ''"
         v-if="getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_DELIVERY_METHOD, this.$const.masterData.DELIVERY) === exchangeData.selected.shipping.sale_delivery_method">
          ...
          <q-card square :flat="selectedAddress === index1" class="fit"
            ...
            <q-card-title>
              ...
              <div v-else="">
                ...
                <div class="text-green-7">{{ exchangeData.selected.shipping.recipient_name }}</div>
                <div class="q-title"><small>{{ exchangeData.selected.shipping.recipient_mobile_phone_number}}</small></div>
              </div>
            </q-card-title>
            <q-card-main>
              ...
              <div class="row fit justify-center">
                <div class="text-grey-3 self-center">
                  <q-field :error-label="$t('Common.address.errorLabel.selectShippingInfo')"
                           :error="validations.exchangeData.selected.shipping.recipient_addresses.$error">
              ...
              <div class="row gutter-sm">
                <div class="col-12">
                  <q-field :label="$t('Common.address.label.recipientName')"
                           :error="validations.exchangeData.selected.shipping.recipient_name.$error">
                    <q-input :disable="lockProductExchange" clearable
                             v-model="exchangeData.selected.shipping.recipient_name"
                             @input="validations.exchangeData.selected.shipping.recipient_name.$touch"
                    />
                  </q-field>
                </div>
                <div class="col-12 relative-position">
                  <q-field :label="$t('Common.address.label.phoneNumber')"
                           :error="validations.exchangeData.selected.shipping.recipient_mobile_country_code_id.$error || validations.exchangeData.selected.shipping.recipient_mobile_phone_number.$error"
                  ...
                  >
                    <div class="row gutter-xs">
                      <div class="col-6">
                        <q-select clearable
                                  :disable="lockProductExchange"
                                  @input="validations.exchangeData.selected.shipping.recipient_mobile_country_code_id.$touch"
                                  v-model="exchangeData.selected.shipping.recipient_mobile_country_code_id"
                      ...
                      <div class="col-6">
                        <q-input
                          clearable
                          @input="validations.exchangeData.selected.shipping.recipient_mobile_phone_number.$touch"
                          :disable="exchangeData.selected.shipping.recipient_mobile_country_code_id === '' || lockProductExchange"
                          v-model="exchangeData.selected.shipping.recipient_mobile_phone_number"/>
                      </div>
                    </div>
                  </q-field>
                  <q-loader :visible="isProcessingSection"></q-loader>
                ...
                <div class="col-lg-12 relative-position" v-else="">
                  <q-field class="animate-blink"
                    ...
                    <q-select
                      filter
                      clearable
                      @input="setCountrySelectionOnSelectedAddress(), validations.exchangeData.selected.shipping.recipient_addresses.$touch"
...
<script>
  ...
  export default {
    ...
    created () {
      ...
      if (this.lockProductExchange && this.exchangeData.selected.shipping.recipient_addresses.length > 0) {
        salesSelectedRecipientAddress = JSON.parse(this.exchangeData.selected.shipping.recipient_addresses)
      }
      ...
      if (this.memberData.information !== null && this.memberData.information !== '' && !this.lockProductExchange) {
        if (this.exchangeData.selected.shipping.recipient_name === '') {
          this.exchangeData.selected.shipping.recipient_name = this.memberData.details.name
        }
        if (this.exchangeData.selected.shipping.recipient_mobile_country_code_id === '') {
          this.exchangeData.selected.shipping.recipient_mobile_country_code_id = this.memberData.contact_info.mobile_1_country_code_id
        }
        if (this.exchangeData.selected.shipping.recipient_mobile_phone_number === '') {
          this.exchangeData.selected.shipping.recipient_mobile_phone_number = this.memberData.contact_info.mobile_1_num
        }
      }
      if (this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_DELIVERY_METHOD, this.$const.masterData.DELIVERY) === this.exchangeData.selected.shipping.sale_delivery_method) {
        this.enableMixinInitCheck = true
      }
      if (this.exchangeData.selected.shipping.recipient_selected_shipping_index !== '') {
        this.selectedAddress = this.exchangeData.selected.shipping.recipient_selected_shipping_index
      }
    },
    mounted () {
      this.master.keys = [
      ...
      if (!this.lockProductExchange && this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_DELIVERY_METHOD, this.$const.masterData.DELIVERY) === this.exchangeData.selected.shipping.sale_delivery_method && this.exchangeData.selected.shipping.recipient_selected_shipping_index !== '') {
        this.selectedAddress = this.exchangeData.selected.shipping.recipient_selected_shipping_index
        this.applyRecipientAddress(this.exchangeData.selected.shipping.recipient_selected_shipping_index)
      }
    },
    validations () {
      let def = {
      ...
      if (this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_DELIVERY_METHOD, this.$const.masterData.DELIVERY) === this.exchangeData.selected.shipping.sale_delivery_method && this.addresses.length > 0 && !this.lockProductExchange) {
        def.addresses = {
          $each: {
            fields: {
              $each: {
    ...
    methods: {
      ...
      applyRecipientAddress (index) {
        if (!this.lockProductExchange) {
          if (this.addresses.length > 0 && index >= 0 && index !== '') {
            this.exchangeData.selected.shipping.recipient_selected_shipping_index = index
            this.exchangeData.selected.shipping.recipient_addresses = []
            this.exchangeData.selected.shipping.recipient_addresses.push(this.addresses[index])
          }
        }
      },
      ...
      initAddressMixin () {
        let status = this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_DELIVERY_METHOD, this.$const.masterData.DELIVERY) === this.exchangeData.selected.shipping.sale_delivery_method
