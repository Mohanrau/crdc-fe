        ...
        <div class="shared-shipping-content"
            ...
            <div class="row bg-grey-1 group justify-start">
                <div class="col-lg-2 address-holder cursor-pointer"
                     :class="{'col-lg-4 no-pointer-events' : lockSalesOrder && !skipDownline && !isIncompleteRentalSalesOrder}"
                    ...
                    <q-card square flat class="fit custom-border"
                            :color="(!lockSalesOrder || skipDownline || isIncompleteRentalSalesOrder) && $v.addresses.$each[index1].fields.$invalid ? 'negative text-white' : selectedAddress === index1 ? 'white text-black' : 'white text-black'">
                        <q-card-title>
                            <q-icon v-if="selectedAddress === index1" color="positive" slot="right"
                                    name="check_circle"/>
                            <div class="col-lg-12"
                                 v-if="(!lockSalesOrder || skipDownline)">
                                <div class="q-subheading">{{ address.title }}</div>
                            </div>
                            <div v-else="">
                        ...
                        <q-card-main>
                            <div class="address-holder" v-if="address.fields.length > 0">
                                <div class="address-field light-paragraph"
                                     :class="(!lockSalesOrder || skipDownline) && $v.addresses.$each[index1].fields.$invalid ? 'text-white' : 'text-grey-8'"
            ...
            <div class="row mt-10"
                 v-if="addresses.length > 0 && ((!lockSalesOrder || isIncompleteRentalSalesOrder) || (lockSalesOrder && skipDownline))">
                <div class="col-lg-5">
                            ...
                            <div class="row gutter-xs">
                                ...
                                <div class="col-12 relative-position">
                                    <q-field label="Phone Number"
                                             :label-width="3"
                                             helper="*"
                                             :error="validations.selected.shipping.recipient_mobile_country_code_id.$error || validations.selected.shipping.recipient_mobile_phone_number.$error"
                    ...
                    <q-card square flat class="full-height">
                        ...
                        <q-card-main>
                            ...
                            <div class="row gutter-sm" :class="{'lock disabled': !address.title.includes('Custom')}">
                                <div class="col-lg-12" v-if="address.fields.length > 0"
                                ...
                                <div class="col-lg-12 relative-position" v-else="">
                                    <q-field class="animate-blink" label="Please choose country first"
                                        ...
                                        <q-select
                                                ...
                                                class="no-shadow custom-input-border"
                                                clearable
                                                @input="setCountrySelectionOnSelectedAddress(), validations.selected.shipping.recipient_addresses.$touch"
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      isIncompleteRentalSalesOrder() {
        return this.salesData.is_rental_sale_order === 1
          && (this.salesData.order_status_id && this.salesData.order_status_id !== this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_ORDER_STATUS, this.$const.masterData.COMPLETED))
          && (this.salesData.order_status_id && this.salesData.order_status_id !== this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_ORDER_STATUS, this.$const.masterData.CANCELED))
      }
    },
    created () {
    ...
    validations() {
      ...
      if (this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_DELIVERY_METHOD, this.$const.masterData.DELIVERY) === this.salesData.selected.shipping.sale_delivery_method
        ...
        && (!this.lockSalesOrder || this.skipDownline || this.isIncompleteRentalSalesOrder)) {
        def.addresses = {
          $each: {
            fields: {
              $each: {
      ...
      deliveryMethodEnabler(value) {
        if (this.salesData.is_rental_sale_order === 1) {
          ...
          if (this.getMasterDataIdByKeyAndTitle(this.$const.masterKey.SALE_DELIVERY_METHOD, this.$const.masterData.WITHOUT_SHIPPING) === value) {
            ...
            if (!this.isSalesExchange && this.salesData.sponsor_member_id === this.salesData.downline_member_id) {
              return true
            }
          }
        }
