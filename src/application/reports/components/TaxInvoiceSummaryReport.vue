<template>
  <div class="general-page-holder">
    <basic-form formName="DailyCollectionReportForm"
                ...
                v-bind:formFieldData="taxInvoiceSummarySearchData"
                urlResetPathName=""
          ...
          <div class="row gutter-md">
            <div class="col-12">
              ...
              >
                <q-select
                  clearable
                  color="black"
                  v-model="taxInvoiceSummarySearchData.country_id"
            ...
            <div class="col-12">
              ...
              >
                <q-select
                  multiple
                  v-model="taxInvoiceSummarySearchData.location_ids"
              ...
              <div class="row gutter-xs  justify-between">
                ...
                <div class="col-lg-6 col-xs-6">
                  <q-field
                    label-width="3"
                    label="  To"
                    :error="$v.taxInvoiceSummarySearchData.invoice_to_date.$error"
                    error-label="Transaction Date to must be later than transaction date from, and cannot be future date"
                  >
                    <q-datetime align="left" monday-first format="DD-MM-YYYY"
                                v-model="taxInvoiceSummarySearchData.invoice_to_date"
                    />
                  </q-field>
                </div>
              </div>
              ...
              <div class="row gutter-xs justify-between">
                <div class="col-lg-6 col-xs-6">
                  <q-field
                    label-width="6"
                    label="CW From"
                    :error="$v.taxInvoiceSummarySearchData.cw_from.$error"
                    error-label="Submission date from must be before submission date to"
                  >
                    <q-select
                      v-model="taxInvoiceSummarySearchData.cw_from"
            ...
            <div class="col-12">
              ...
              >
                <q-select
                  clearable
                  color="black"
                  v-model="taxInvoiceSummarySearchData.invoice_status"
                  :options="masterDataOptionsCreator($const.masterKey.SALE_ORDER_STATUS)"
                ></q-select>
              </q-field>
            </div>
            <div class="col-lg-12 col-md-12 col-xs-12">
              <q-list no-border class="col-12">
                <q-field
                  :error="$v.taxInvoiceSummarySearchData.user_ids.$error && !$v.taxInvoiceSummarySearchData.user_ids.required"
                ...
                >
                  ...
                  <q-item v-for="(user, index) in taxInvoiceSummarySearchData.user_ids" :key="index">
                    <q-item-main>
                      <q-item-tile>
                        <q-field
                          :error="$v.taxInvoiceSummarySearchData.user_ids.$each[index].$error || setError(user.user_id,'type.exp')"
                        ...
                        >
                          <q-input v-model="user.user_name"
                                   ...
                                   @change="$v.taxInvoiceSummarySearchData.user_ids.$each[index].$touch()"
                          >
                            <q-autocomplete
              ...
              <div class="col-12">
                <q-btn
                  ...
                  round
                  icon="add"
                  :disable="taxInvoiceSummarySearchData.user_ids.length >= 10"
            ...
            <!--<div class="col-12">-->
              ...
              <!--&gt;-->
                <!--<div class="row gutter-xs">-->
                  <!--<div class="col-11">-->
                    <!--<q-input-->
                      <!--v-model="taxInvoiceSummarySearchData.user_name"-->
                      <!--clearable-->
                      <!--:disable="taxInvoiceSummarySearchData.hasUserSelected"-->
                      <!--:placeholder="$t('bonus.yearlyIncome.placeholder.searchMember')"-->
                    <!--&gt;-->
                      <!--<q-autocomplete-->
...
<script>
  ...
  export default {
    ...
    mounted: function () {
      this.taxInvoiceSummarySearchData.country_id = this.selectedFilters.countryID
      this.taxInvoiceSummarySearchData.invoice_from_date = this.getCurrentDate()
      this.taxInvoiceSummarySearchData.invoice_to_date = this.getCurrentDate()
      this.master.keys = [this.$const.masterKey.YEARLY_REPORT_TYPE, this.$const.masterKey.SALE_ORDER_STATUS]
    ...
    methods: {
      ...mapActions([
        ...
        'getTaxInvoiceSummaryReportAction',
      ...
      selected (item, user, field, keyboard) {
        if (!keyboard) {
          ...
          if (field === 'user_id') {
            ...
            this.taxInvoiceSummarySearchData.cw_id = item
            this.$v.taxInvoiceSummarySearchData.cw_name.$touch()
          }
        }
      },
      getAdditionalIBO () {
      ...
      addMemberInSearch () {
        this.taxInvoiceSummarySearchData.user_ids.push(this.getAdditionalIBO())
      },
      removeMemberInSearch (user, index) {
        ...
        this.$v.taxInvoiceSummarySearchData.user_ids.$reset()
        this.taxInvoiceSummarySearchData.user_ids.splice(this.taxInvoiceSummarySearchData.user_ids.indexOf(user), 1)
      },
      getCommissionWeeksData () {
        ...
        this.getCommissionWeekSearchAction(query).then(() => {
          if (this.commissionWeeks.length > 0) {
            this.taxInvoiceSummarySearchData.cw_from = this.commissionWeeks[0].value
            this.taxInvoiceSummarySearchData.cw_to = this.commissionWeeks[0].value
          }
        })
      },
      processForm () {
        this.$v.taxInvoiceSummarySearchData.$touch()
        return new Promise((resolve, reject) => {
          if (!this.$v.taxInvoiceSummarySearchData.$error) {
            let user_id_array = this.taxInvoiceSummarySearchData.user_ids.map((user) => user.user_id)
            let query = {
              country_id: this.taxInvoiceSummarySearchData.country_id,
              location_ids: this.taxInvoiceSummarySearchData.location_ids,
              from_date: this.formatDate(this.taxInvoiceSummarySearchData.invoice_from_date, 'YYYY-MM-DD'),
              to_date: this.formatDate(this.taxInvoiceSummarySearchData.invoice_to_date, 'YYYY-MM-DD'),
              from_cw: this.taxInvoiceSummarySearchData.cw_from,
              to_cw: this.taxInvoiceSummarySearchData.cw_to,
              ibo_ids: user_id_array,
              status: this.taxInvoiceSummarySearchData.invoice_status
            }
            this.getTaxInvoiceSummaryReportAction(query).then((res) => {
              let pdfUrl = res
            ...
            reject(this.$v.taxInvoiceSummarySearchData.$error)
          }
        })
      }
    }
