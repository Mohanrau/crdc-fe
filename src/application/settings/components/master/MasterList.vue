<template>
    <div class="general-page-holder">
        <general-header generalTitle="List of available Master">
...
<script>
  import createMasterForm from './forms/createMasterForm'
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            format (value) {
              if (value === 1) {
                return '<i class="material-icons text-positive" style="font-size: 22px">account_circle</i>'
              } else {
                return '<i class="material-icons text-negative" style="font-size: 22px">account_circle</i>'
              }
            }
          },
          {
    ...
    methods: {
      ...
      openForm () {
        this.$router.replace('/masters/master/create')
      },
      // Query is optional parameter
      // Default ?limit=10&offset=0&sort=&order=
      queryHandler () {
        Vue.nextTick(() => {
          this.getMasterListAction(this.paginationQuery)
        })
      },
      ...
      actions (row, type) {
        if (type === 'create') {
          this.$router.push('/masters/master/create')
        } else if (type === 'update') {
          this.$router.push('/masters/master/' + row.id + '/update')
        } else if (type === 'delete') {
          Dialog.create({
            title: 'Warning',
            ...
            buttons: [
              this.$t('Common.DeleteCancel.Btn'),
              {
                ...
                handler: () => {
                  this.deleteMasterAction(row.id).then(response => {
                    this.queryHandler()
